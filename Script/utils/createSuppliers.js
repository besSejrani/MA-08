...
export const createSuppliers = (nomberRows) => {
  ...
  for (let i = 1; i <= nomberRows; i++) {
    ...
    const suppliers = `insert into suppliers (name, email, address, phone, country) VALUES('${faker.company
      .companyName()
      .replace("'", "")
      .replace("'", "")}', '${faker.internet.email()}', '${faker.address
      .streetAddress()
      .replace("'", "")}', '${faker.phone.phoneNumber().replace("'", "")}', '${faker.address
      .country()
      .replace("'", "")}')`;
